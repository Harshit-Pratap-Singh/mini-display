...
## Working style
- **Git identity: personal, not work.** This is a personal project, so it commits as `harshit pratap singh <singhharshitpratap@gmail.com>`, set in `.git/config` (local) so the global work identity is untouched. History was rewritten to this author on 2026-09-20; the fork's root commit by Kame (iodn) was deliberately left alone. `.git/config` is not cloned, so after a fresh clone re-run: `git config --local user.name "harshit pratap singh" && git config --local user.email "singhharshitpratap@gmail.com"`.
