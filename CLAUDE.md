...
- **Three selectable faces**, same pattern as the clock faces: `1` album art + telemetry, `2` low-RAM text-first, `3` radial ring. Designs came from the user's mockups.
...
- **Fixed DESIGN.md palette on the Spotify faces** (cyan `#38BDF8`, amber `#ffb95f`, mint `#56e5a9` on `#111318`) rather than `activeTheme()`. This is the one page that ignores the theme picker.
- **No spectrum/EQ visualiser.** The device has no audio — it reports what is playing elsewhere, so any bars would be invented. That strip shows **real telemetry** instead (WiFi signal, free heap, poll latency), which is what the design language is for.
