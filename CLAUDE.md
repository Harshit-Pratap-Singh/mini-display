...
- **Modes = iodn page chips** (`enabledPages[]` in `/dashboard-config.json`, edited with live preview on the dashboard). After the strip the chips are Clock, Weather, later Photos (M5) and Spotify (M6). One ticked chip = fixed mode; several + "Auto rotate" = timed cycle (`rotationIntervalSec`); several with rotate off = manual.
...
3. **Network + clock mode.** Verify the base firmware's WiFi setup, config storage, NTP and OTA on our board (ArduinoOTA and mDNS are gated on ≥28,000 B free heap — `kOptionalServiceMinFreeHeapBytes`, `src/main.cpp` — and never start on this board until the gate is lowered; only web `/update` is testable first); restyle its clock page to our layout; replace button mode cycling with the button-less scheme in "Mode logic".
